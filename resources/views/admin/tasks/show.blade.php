...
<div class="container py-4">
    ...
    @php
        ...
        $completedTasks = $allTasks->filter(function($t) {
            return in_array(strtolower($t->status), ['done', 'completed', 'selesai']);
        })->count();
        $inProgressTasks = $allTasks->filter(function($t) {
            return strtolower($t->status) === 'in_progress';
        })->count();
        $todoTasks = $allTasks->filter(function($t) {
            return strtolower($t->status) === 'to_do';
        })->count();
    @endphp
